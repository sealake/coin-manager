...
import net.sealake.coin.util.Messages;
...
public class AppResponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  protected ResponseEntity<Object> handleRequestInvalidException(final BadRequestException ex, //
      final WebRequest request) {
  ...
  protected ResponseEntity<Object> handleAccessDeniedException(final Exception ex, final WebRequest request) {
    ...
    final ErrorMessage errorMessage = //
        new ErrorMessage(AppError.PERMISSION_DENIED.getErrorCode(), ex.getMessage());
    return new ResponseEntity<Object>(errorMessage, status);
  }
